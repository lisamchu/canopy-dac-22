...
mini_facet = bayes_tidy2 %>% 
  ...
  arrange(desc(abs(estimate))) %>%
  slice(1:10) %>% 
  mutate(plot_labs = case_when(nice_demog == "% English Language Learner" ~ "Schools with higher-than-average % English learners",
...
mini_plots = 
  ...
  labs(y = "Odds multiplier",
       x = "", 
       title = "Top 10 practices with the strongest relationship to\nfour student demographic groups",
  ...
  facet_wrap(~ plot_labs, scales = "free_y",
             labeller = labeller(plot_labs = label_wrap_gen(width = 25))) +
  scale_y_continuous(
    trans = "log",
    breaks = c(.125, .25, .5, 1, 2, 4, 8),
    labels = c("1/8", "1/4", "1/2", "1", "2", "4", "8"),
  ...
  coord_flip() +
  guides(fill = "none") +
  theme(axis.text = element_text(size = 10), strip.text = element_text(size = rel(.8)),
        panel.grid.major.y = element_blank(),
        axis.text.x = element_text(angle = 0, vjust = 0.5, hjust = 0),
...
ggsave("frpl-mod2-mini-plot.png", plot = frpl_plot, path = here("outputs", "type analysis"),
       width = 12, height = 8, units = "in")
# facet minis
ggsave_transcend(mini_plots, file = "figure 9", fig_height = 9, fig_width = 12)
```
