...
##save data for Figure H
# var_max_10_dat <- var_all %>% 
#   group_by(type) %>% 
#   slice_max(variance, n = 10, with_ties = FALSE)
# export(var_max_10_dat, "fig_h_data.csv")

## more fixing up for readability

var_max_10_edit <- var_all %>% 
  group_by(type) %>% 
  slice_max(variance, n = 10, with_ties = FALSE) %>% 
  ggplot(aes(tag, proportion, fill = type)) + 
...
Plot found in `outputs` > `type analysis` > `model-race-ses.png`. Code takes a long time to run, so I don't recommend pulling from this document.
...
mod_prep <- clean_data_full %>% 
  select(school_id, starts_with("practices_"), ends_with("_percent"), self_reported_total_enrollment, nces_total_enrollment) %>% 
  left_join(type_recode, by = "school_id") %>% 
  ...
  unite("n_student", c(nces_total_enrollment, self_reported_total_enrollment), na.rm = TRUE) %>% 
  mutate(n_student = na_if(n_student, ""),
         n_student = str_remove_all(n_student, "_..."),
         n_student = as.integer(n_student)) %>% 
  pivot_longer(cols = starts_with("practices"),
               names_to = "tag",
               values_to = "value") %>% 
  select(-type)
...
one_vars = c("n_student", "black_percent", "hispanic_percent", "school_descriptor_charter", "school_descriptor_district", "school_descriptor_independent", "frpl_percent", "swd_percent", "ell_percent")
...
Plot found in `outputs` > `type analysis` > `model-bipoc-ses.png`. Code takes a long time to run, so I don't recommend pulling from this document.
...
##from Gregor's code:
#first will run black percent and hispanic percent as controls
mod2_vars = c("non_white_percent", "school_descriptor_charter", "school_descriptor_district", "school_descriptor_independent", "frpl_percent", "swd_percent", "ell_percent", "n_student")

logistic_dat2 = 
  mod2_prep %>%
  select(value, tag, one_of(mod2_vars)) %>%
  na.omit() %>%
  group_by(tag) %>%
  # scaling non-binary predictors
  mutate_at(vars(
...
mod2_form = as.formula(paste("value", "~", paste(mod2_vars, collapse = "+")))
#plot(xm, "areas", prob = 0.9, prob_outer = 1)

bayes_mods2 = list()

for(this_tag in unique(mod2_prep$tag)) {
  bayes_mods2[[this_tag]] = stan_glm(
    mod2_form,
    data = filter(logistic_dat2, tag == this_tag),
    family = binomial(link = "logit"),
    prior = student_t(
      df = 7,
      location = 0,
...
bayes_tidy2 = lapply(bayes_mods2, tidy) %>%
  ...
  filter(term != "(Intercept)") %>%
  mutate(
    nice_tag = label_tags(response, width = 30),
    #nice_tag = label_tags(response),
    nice_demog = factor(label_dems(term))
  )
...
Re-create plots for SWD, ELL, BIPOC %, and FRPL eligible using Transcend colors (each one can use a different color).

```{r model 2 mini plots, fig.dim = c(9, 9)}
# SWD plot
swd_plot <- bayes_facet_dat2 %>% 
  filter(term == "swd_percent") %>% 
  ggplot(aes(y = exp(estimate), 
             x = reorder_within(nice_tag, estimate, within = nice_demog, fun = mean))) +
  geom_col(fill = "#EF464B") + 
  labs(y = "Odds multiplier",
       x = "", 
       title = "",
       subtitle = "% Students with Disabilities") +
  scale_y_continuous(
    trans = "log",
    breaks = c(.125, .25, .5, 1, 2, 4, 8),
    labels = c("1/8", "1/4", "1/2", "1", "2", "4", "8"),
...
# ELL plot
ell_plot <- bayes_facet_dat2 %>% 
  filter(term == "ell_percent") %>% 
  ggplot(aes(y = exp(estimate), 
             x = reorder_within(nice_tag, estimate, within = nice_demog, fun = mean))) +
  ...
  labs(y = "Odds multiplier",
       x = "", 
       title = "",
       subtitle = "% English Language Learner") +
  scale_y_continuous(
    trans = "log",
    breaks = c(.125, .25, .5, 1, 2, 4, 8),
    labels = c("1/8", "1/4", "1/2", "1", "2", "4", "8"),
...
# BIPOC plot
bipoc_plot <- bayes_facet_dat2 %>% 
  filter(term == "non_white_percent") %>% 
  ggplot(aes(y = exp(estimate), 
             x = reorder_within(nice_tag, estimate, within = nice_demog, fun = mean))) +
...
# FRPL plot
frpl_plot <- bayes_facet_dat2 %>% 
  filter(term == "frpl_percent") %>% 
  ggplot(aes(y = exp(estimate), 
             x = reorder_within(nice_tag, estimate, within = nice_demog, fun = mean))) +
  geom_col(fill = "#ADE0EE") + 
  labs(y = "Odds multiplier",
       x = "", 
       title = "",
       subtitle = "% FRPL eligible") +
  scale_y_continuous(
    trans = "log",
    breaks = c(.125, .25, .5, 1, 2, 4, 8),
    labels = c("1/8", "1/4", "1/2", "1", "2", "4", "8"),
...
mini_facet = bayes_tidy2 %>% 
  ...
  filter(term == "swd_percent" |
         term == "ell_percent" |
         term == "frpl_percent" |
         term == "non_white_percent") %>% 
  mutate(plot_labs = case_when(nice_demog == "% English Language Learner" ~ "Schools with higher-than-average % English language learners",
                               nice_demog == "% FRPL eligible" ~ "Schools with higher-than-average % students eligible for free/reduced-price lunch",
                               nice_demog == "% students of color" ~ "Schools with higher-than-average % BIPOC students",
                               nice_demog == "% Students with Disabilities" ~ "Schools with higher-than-average % students with disabilities"))

mini_plots = 
  ggplot(mini_facet,
    aes(y = exp(estimate),
        x = reorder_within(nice_tag, estimate, within = plot_labs, fun = mean),
    )) +
  geom_col(aes(fill = term), fill = c(paste(rep("#EF464B", 10)),
                                      paste(rep("#1A4C81", 10)),
                                      paste(rep("#59C3B4", 10)),
                                      paste(rep("#ADE0EE", 10)))) + 
  labs(y = "Likelihood to report",
       x = "", 
       title = "Top 10 practices with the strongest relationship to four student demographic groups") +
       # caption = "Note: An odds multiplier over 1 indicates that schools were more likely than others to report the practice (e.g.,schools\nwith higher-than-average BIPOC student populations were over twice as likely to report culturally-responsive practices).\nAn odds multiplier under 1 indicates that schools were less likely than others to report the practice.") +
  facet_rep_wrap(~ plot_labs, scales = "free_y",
                 repeat.tick.labels = "bottom",
                 labeller = labeller(plot_labs = label_wrap_gen(width = 33))) +
  scale_y_continuous(
    trans = "log",
    breaks = c(.125, .25, .5, 1, 2, 4, 8),
    labels = c("1/8", "1/4", "1/2 as likely", "", "2x more likely", "4", "8"),
    expand = expansion(0, 0)
  ) +
  scale_x_reordered(breaks = ) + 
  coord_flip() +
  guides(fill = "none") +
  theme(axis.text = element_text(size = 9), strip.text = element_text(size = rel(.8)),
        panel.grid.major.y = element_blank(),
        axis.text.x = element_text(angle = 0, vjust = 0.5, hjust = 0.5), #hjust center align at 0.5 (0 before)
        plot.caption = element_text(hjust = 0, size = 11)) +
  # scale_x_discrete(labels = scales::label_wrap(15)) +
  geom_hline(yintercept = 1) +
  expand_limits(y = c(1/3, 3))

mini_plots

# save plots
# SWD
ggsave("swd-mod2-mini-plot.png", plot = swd_plot, path = here("outputs", "type analysis"),
       width = 12, height = 8, units = "in")
# ELL
ggsave("ell-mod2-mini-plot.png", plot = ell_plot, path = here("outputs", "type analysis"),
       width = 12, height = 8, units = "in")
# BIPOC %
ggsave("bipoc-mod2-mini-plot.png", plot = bipoc_plot, path = here("outputs", "type analysis"),
       width = 12, height = 8, units = "in")
# FRPL
ggsave("frpl-mod2-mini-plot.png", plot = frpl_plot, path = here("outputs", "type analysis"),
       width = 12, height = 8, units = "in")
# facet minis
ggsave_transcend(mini_plots, file = "figure 9", fig_height = 9, fig_width = 14)
```


## Controlling for Locale  

Predictors: bipoc percent, number of students, school type, school locale, FRPL percent, ELL percent, and SWD percent  

Plot found in `outputs` > `type analysis` > `model-type-locale.png`. Code takes a long time to run, so I don't recommend pulling from this document.

```{r model 3 - locale, eval = FALSE}
#data prep
mod3_prep <- clean_data_full %>% 
  select(school_id, starts_with("practices_"), ends_with("_percent"), locale, nces_total_enrollment, self_reported_total_enrollment) %>% 
  left_join(type_recode, by = "school_id") %>% 
  mutate(type = factor(.$type, levels = c("charter", "district", "independent"), labels = c("Charter", "District", "Independent"))) %>% 
  unite("n_student", c(nces_total_enrollment, self_reported_total_enrollment), na.rm = TRUE) %>% 
  mutate(n_student = na_if(n_student, ""),
         n_student = str_remove_all(n_student, "_..."),
         n_student = as.integer(n_student)) %>% 
...
##from Gregor's code:
#use non-white percent (mainly to limit vars - can change)
mod3_vars = c("non_white_percent", "n_student", "locale", "type", "frpl_percent", "locale:type") # "ell_percent", "swd_percent",

logistic_dat3 = 
  mod3_prep %>%
  select(value, tag, one_of(mod3_vars)) %>%
  na.omit() %>%
  group_by(tag) %>%
  # scaling non-binary predictors
  mutate_at(vars(
    c(ends_with("_percent"), n_student)), scale)

mod3_form = as.formula(paste("value", "~", paste(mod3_vars, collapse = "+")))
#plot(xm, "areas", prob = 0.9, prob_outer = 1)

bayes_mods3 = list()

for(this_tag in unique(mod3_prep$tag)) {
  bayes_mods3[[this_tag]] = stan_glm(
    mod3_form,
    data = filter(logistic_dat3, tag == this_tag),
    family = binomial(link = "logit"),
    prior = student_t(
      df = 7,
      location = 0,
...
bayes_facet3 = 
  ...
  labs(y = "Odds multiplier",
       ...
       #fill = "Demographic",
       title = "Top 10 tags most/least related to each demographic",
       subtitle = "as compared to urban charter schools") +
  facet_wrap(~ nice_demog, scales = "free_y") +
  scale_y_continuous(
    trans = "log",
    breaks = c(.125, .25, .5, 1, 2, 4, 8),
  ...
  theme(axis.text = element_text(size = 8), strip.text = element_text(size = rel(0.6)),
        ...
        axis.text.x = element_text(angle = 0, vjust = 0.5, hjust = 0)) +
  geom_hline(yintercept = 1) #+
  #expand_limits(y = c(1/9, 4))

bayes_facet3
```

## TO DO: *redo model 3 including ELL % and SWD %*
Considerations:  
* number of variables makes facet wrap difficult  
* how to expand facet plot so all 12 plots show?  
* alternately print separately?  
ALTERNATE: *redo model 3 using an interaction term b/w type & locale*
* need to figure out how to modify the titles on the facet plots, but otherwise probs ready to go


## TO DO: Model using cluster weights as outcomes  
* remember weights need to be standardized -- may want to double check if Gregor's document included a standardized variable for cluster weights or not

```{r cluster model}
#pulled from `cluster_correlations.rmd`
...
names(c5)[1] <- "tag"
#might be better to just read in tsv file created after joining with practices and schools
# need to decide which will be better to incorporate into the model
# and then how to merge it with existing modeling data
cluster_mod_prep <- mod_prep3 %>% 
  left_join(c5, by = "tag") %>% #ignore this - you do not need to merge with practices, your DV is the clusters


```

## TO DO: other questions

DEMOGRAPHICS: How do tagging patterns differ by student demographics (%white, %Black, %Hispanic/Latino, %Asian, %Native)?

```{r racial demo only mod}
##from Gregor's code:
#use all 3 racial categories
vars4 = c("aian_percent", "asian_percent", "black_percent", "hispanic_percent", "nhpi_percent", "multiple_percent", "white_percent")

logistic_one_dat4 = 
  mod_prep3 %>%
  select(value, tag, one_of(vars4)) %>%
  na.omit() %>%
  group_by(tag) %>%
  # scaling non-binary predictors
  mutate_at(vars(
...
one_form4 = as.formula(paste("value", "~", paste(vars4, collapse = "+")))
#plot(xm, "areas", prob = 0.9, prob_outer = 1)

bayes_mods4 = list()

for(this_tag in unique(mod_prep3$tag)) {
  bayes_mods4[[this_tag]] = stan_glm(
    one_form4,
    data = filter(logistic_one_dat4, tag == this_tag),
    family = binomial(link = "logit"),
    prior = student_t(
      df = 7,
      location = 0,
...
bayes_tidy4 = lapply(bayes_mods4, tidy) %>%
  bind_rows(.id = "response") %>%
  filter(term != "(Intercept)") %>%
  mutate(
    nice_tag = label_tags(response),
...
bayes_facet_dat4 = bayes_tidy4 %>% group_by(nice_demog) %>%
  arrange(desc(abs(estimate))) %>%
  slice(1:10)

bayes_facet4 = 
  ggplot(bayes_facet_dat4,
    aes(y = exp(estimate),
        x = reorder_within(nice_tag, estimate, within = nice_demog, fun = mean),
    )) +
  geom_col() + 
...
ggsave("model-race-ses.png", plot = bayes_facet, path = here("outputs", "type analysis"), 
       width = 20, height = 12, units = "in")
#nonwhite/bipoc percent as variable in odds model
ggsave("model-bipoc-ses.png", plot = bayes_facet2, path = here("outputs", "type analysis"), 
       width = 20, height = 12, units = "in")
#racial vars + locale
ggsave("model-type-locale.png", plot = bayes_facet3, path = here("outputs", "type analysis"), 
       width = 20, height = 12, units = "in")
```
...
```{r size by type, fig.dim = c(9, 9)}
#table with mean and median for each group
type_size <- type_desc %>% 
  group_by(type) %>% 
  mutate(student_tot = as.integer(student_tot),
         mean = round(mean(student_tot, na.rm = TRUE), 2),
         median = median(student_tot, na.rm = TRUE)) %>% 
  select(type, mean, median) %>% 
  unique()
datatable(type_size)

#viz for type - violin plot
type_size_viz <- type_size <- type_desc %>% 
  group_by(type) %>% 
  mutate(student_tot = as.integer(student_tot),
         mean = round(mean(student_tot, na.rm = TRUE), 2)) %>% 
  ggplot(aes(y = student_tot, x = type, fill)) +
  geom_violin(fill = transcend_cols["teal"]) +
  geom_point(color = transcend_cols["red"],
             position = position_jitter(width = .2),
             size = 0.5) +
  labs(x = "",
       y = "number of students",
       title = "Student Population across School Type",
       subtitle = "Wider areas indicate more schools with given student population") +
    # theme(axis.text.x = element_blank(),
    #       strip.text = element_text(size = rel(0.5)),
    #       plot.subtitle = element_text(size = rel(0.7)))
  scale_y_continuous(expand=c(0,0)) +
  scale_x_discrete(labels=c("charter" = "Charter Schools \nmean = 299.5", 
...
Curious about the size outliers in district schools, are these P-12 or not?
...
```{r save type size plot}
#racial vars + locale
ggsave("type-school-size.png", plot = type_size_viz, path = here("outputs", "type analysis"), 
       width = 12, height = 8, units = "in")
```
...
```{r n practices hist, fig.dim = c(12, 9)}
avg_prac <- clean_data_full %>% 
  select(school_name, starts_with("practices_")) %>% 
  mutate(n_practices = rowSums(across(where(is.numeric))),
         avg_practices = mean(n_practices,)) %>% 
#hist
ggplot(., aes(n_practices)) +
  geom_bar() +
  theme_transcend +
  geom_vline(xintercept = 38.77019, #average number of tags `$avg_practices`
             color = "#EF464B", size = 1, linetype = "dashed") +
  scale_y_continuous(expand = expansion(add = c(0, 1))) +
  annotate("text", x = 39, y = 9.5, 
          label = "— average practices selected = 39", color = "#EF464B",
          size = 4, fontface="bold", hjust = 0) + #angle=45
  labs(x = "Number of Practices Selected",
       y = "Number of Schools",
       title = "How many practices did schools typically select?",
       subtitle = "dashed line indicates average number of practices selected")
```

```{r save n practices hist, eval = FALSE}
#how many practices did schools typically select?
ggsave("n-practices.png", plot = avg_prac, path = here("outputs", "misc"), 
       width = 12, height = 9, units = "in")
```

## Were more students co-leading in public schools than in independent schools?

The following table shows combined student enrollment total by type for schools that selected the tag `students as co-leaders`.

```{r co-lead totals}
colead <- type_recode %>% 
  left_join(practices_data, by = "school_id") %>% 
  select(school_id, type, practices_students_leaders) %>% 
  left_join(type_desc, by = c("school_id", "type")) %>% 
  select(school_id, type, practices_students_leaders, student_tot) %>% 
  filter(practices_students_leaders == 1) %>% 
  mutate(student_tot = as.integer(student_tot)) %>% 
  group_by(type) %>% 
  summarize(grand_tot = sum(student_tot)) %>% 
  rename(`School Type` = type,
         `Total Student Enrollment` = grand_tot)
datatable(colead)
```
