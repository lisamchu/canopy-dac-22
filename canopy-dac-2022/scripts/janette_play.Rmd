...
```{r collapse school type, eval = FALSE}
#district c131
#charter c132
#independent c133
#sws c134
...
```{r nces race percentages, eval = FALSE}
...
```{r merge race percentages, eval = FALSE}
clean_dac_22 <- clean_dac_22 %>% 
...
clean_dac_22 <- clean_dac_22 %>% 
  ...
  mutate_at(vars(starts_with("grades_"), -grades_other), str_replace, "Yes_..", "Yes") %>% 
...
```{r new school type variable, eval = FALSE}
#create new column
clean_dac_22 <- clean_dac_22 %>% 
...
```{r pull out multicoded schools, eval = FALSE}
#create a table with schools that are multicoded
multi_code_grades <- clean_dac_22 %>% 
  select(school_id, school_name, school_grade_type,
         grades_pre_k, grades_kindergarten, grades_1st, grades_2nd, grades_3rd, grades_4th, grades_5th, grades_6th, grades_7th, grades_8th, grades_9th, grades_10th, grades_11th, grades_12th, grades_past_12th, grades_other) %>% 
...
**Update:** School types have updated scheme:  
- 3 cols  
- ANY 1-5 = `elementary`  
- ANY 7&8 = `middle`  
- ANY 10-12 = `high`  
...
```{r, eval = FALSE}
#import data - 'clean' data united, so create new small set to merge
grades <- import(here("data", "dac_22_updated.csv"))
#have to re-merge NCES and self-report data
grades <- grades %>% 
  mutate_at(vars(starts_with("nces_grades_")), na_if, "") %>% 
  mutate_at(vars(starts_with("nces_grades_")), str_replace, "[:digit:][:digit:]", "1") %>% 
  mutate_at(vars(starts_with("nces_grades_")), str_replace, "[:digit:]", "1") %>% 
  mutate_at(vars(starts_with("nces_grades_")), str_replace, "Yes", "1") %>% 
  mutate_at(vars(starts_with("nces_grades_")), na_if, "Not reported") %>% 
  mutate_at(vars(starts_with("nces_grades_")), str_replace, "No", "0") %>% 
  unite("grades_pre_k", c(nces_grades_pre_k, self_reported_grades_pre_k), na.rm = TRUE) %>% 
  unite("grades_kindergarten", c(nces_grades_kindergarten, self_reported_grades_kindergarten), na.rm = TRUE) %>% 
  unite("grades_1st", c(nces_grades_1st, self_reported_grades_1st), na.rm = TRUE) %>% 
  unite("grades_2nd", c(nces_grades_2nd, self_reported_grades_2nd), na.rm = TRUE) %>% 
  ...
  unite("grades_12th", c(nces_grades_12th, self_reported_grades_12th), na.rm = TRUE) %>% 
  unite("grades_past_12th", c(nces_grades_13th, self_reported_grades_past_12th), na.rm = TRUE) %>% 
  rename(grades_other = self_reported_grades_other,
         grades_other_text = self_reported_grades_other_text)

grades <- grades %>% 
  select(school_id, grades_pre_k, grades_kindergarten, grades_1st, grades_2nd, grades_3rd, grades_4th, grades_5th, grades_6th, grades_7th, grades_8th, grades_9th, grades_10th, grades_11th, grades_12th, grades_past_12th, grades_other, grades_other_text) %>% 
  mutate_at(vars(starts_with("grades")), na_if, "") %>% 
  mutate_at(vars(starts_with("grades")), str_remove_all, "_.") %>% 
  group_by(school_id) %>% 
  mutate(grades_elementary = ifelse(grades_1st == "1" &
         ...
         grades_high = ifelse(grades_10th == "1" &
                       grades_11th == "1" &
                       grades_12th == "1", "1", "0"))
#prepare clean set to merge
merge_grades <- import(here("data", "clean_dac_22.csv")) %>% 
  select(-c(grades_pre_k, grades_kindergarten, grades_1st, grades_2nd, grades_3rd, grades_4th, grades_5th, grades_6th, grades_7th, grades_8th, grades_9th, grades_10th, grades_11th, grades_12th, grades_past_12th, grades_other, school_grade_type)) %>% 
  left_join(grades, by = "school_id")

View(colnames(merge_grades))
#export(merge_grades, "clean_dac_22.csv")
```
\newpage
...
View(dat_chk$confidential_leader_race_self_describe)

# dat_chk <- dat_chk %>%
#   select(-c(grades_other_text.y, grades_elementary.y, grades_middle.y, grades_high.y))
# 
# dat_chk <- dat_chk %>%
#   rename(grades_other_text = grades_other_text.x,
#          grades_elementary = grades_elementary.x,
#          grades_middle = grades_middle.x,
#          grades_high = grades_high.x)
# colnames <- data.frame(colnames(dat_chk))
dic <- import(here("data", "Final Canopy Dataset for DAC - Data dictionary.csv"))
#export(colnames, "colnames.csv")
export()
```
\newpage
...
```{r expand practices, eval = FALSE}
#import data
practices <- import(here("data", "clean_dac_22.csv"))
#subset data
practices <- practices %>% 
...
core <- practices %>% 
  ...
  mutate_at(vars(starts_with("core_")), str_replace, "0-2 years", "1") %>% 
...
prac_prep <- practices %>% 
  ...
  mutate_at(vars(starts_with("practices_")), str_replace, "0-2 years", "1") %>% 
  mutate_at(vars(starts_with("practices_")), str_replace, "3-4 years", "1") %>% 
  mutate_at(vars(starts_with("practices_")), str_replace, "5.......", "1")
#merge all subsets
merge_pracs <- left_join(prac_prep, core, by = "school_id") %>% 
  left_join(time, by = "school_id")

#prep clean data by dropping all practices columns
clean_prac <- import(here("data", "clean_dac_22.csv"))
#drop all practices columns
clean_prac <- clean_prac %>% 
  select(-c(starts_with("practices_")))
#remerge expanded columns
clean_merge_prac <- left_join(clean_prac, merge_pracs, by = "school_id")
#export(clean_merge_prac, "clean_dac_22_full.csv")
```


### Collapse ELL variables

NCES and self-reported data for ELL students was merged, with NCES data taking precedence. Final data reflects percentages rounded to 2 decimal points.

*Status:* Complete

```{r collapse ELL, eval = FALSE}
ell <- import(here("data", "clean_dac_22_full.csv"))

ell <- ell %>% 
  select(school_id, self_reported_ell, self_reported_total_enrollment, nces_ell) %>% 
  mutate(self_reported_ell = round(self_reported_ell/self_reported_total_enrollment, 2),
         nces_ell = round(nces_ell, 2)) %>%
  select(-c(self_reported_total_enrollment)) %>% 
  unite("ell_percent", c(nces_ell, self_reported_ell), na.rm = TRUE) %>% 
  mutate(ell_percent = str_remove(ell_percent, "_.."),
         ell_percent = na_if(ell_percent, ""))

ell_merge_prep <- import(here("data", "clean_dac_22_full.csv"))

ell_merge_prep <- ell_merge_prep %>% 
  select(-c(self_reported_ell, nces_ell))

ell_merge <- ell_merge_prep %>% 
  left_join(ell, by = "school_id")
#export data with ell percentage
#export(ell_merge, "clean_dac_22_full.csv")
```
...
```{r collapse FRPL}
frpl <- import(here("data", "clean_dac_22_full.csv"))

frpl <- frpl %>% 
  select(school_id, self_reported_frpl, self_reported_total_enrollment, nces_frpl, nces_total_enrollment) %>% 
  mutate(self_reported_frpl = round(self_reported_frpl/self_reported_total_enrollment, 2),
         nces_frpl = round(nces_frpl/nces_total_enrollment, 2)) %>% 
  select(-c(self_reported_total_enrollment, nces_total_enrollment)) %>% 
  unite("frpl_percent", nces_frpl, self_reported_frpl, na.rm = TRUE)

frpl <- frpl %>% 
  mutate(frpl_percent = na_if(frpl_percent, ""))

frpl_merge_prep <- import(here("data", "clean_dac_22_full.csv"))

frpl_merge_prep <- frpl_merge_prep %>% 
  select(-c(self_reported_frpl, nces_frpl))

frpl_merge <- frpl_merge_prep %>% 
  left_join(frpl, by = "school_id")
#export merged data with new column
#export(frpl_merge, "clean_dac_22_full.csv")
```
...
```{r collapse SWD}
swd <- import(here("data", "clean_dac_22_full.csv"))

swd <- swd %>% 
  select(school_id, self_reported_swd, self_reported_total_enrollment, nces_swd) %>% 
  mutate(self_reported_swd = round(self_reported_swd/self_reported_total_enrollment, 2),
         nces_swd = round(nces_swd, 2)) %>% 
  select(-c(self_reported_total_enrollment))
#collapse
swd <- swd %>% 
  unite("swd_percent", nces_swd, self_reported_swd, na.rm = TRUE)
#missing values
swd <- swd %>% 
  mutate(swd_percent = na_if(swd_percent, ""))

swd_merge_prep <- import(here("data", "clean_dac_22_full.csv")) %>% 
  select(-c(self_reported_swd, nces_swd))

swd_merge <- swd_merge_prep %>% 
  left_join(swd, by = "school_id")
#export new file
#export(swd_merge, "clean_dac_22_full.csv")
```


### Organize Clean Data

Ensure all related columns are grouped together.

*Status:* Complete.

```{r organize}
org <- import(here("data", "clean_dac_22_full.csv"))
#reorder rows --saved as "wide" format
org <- org %>% 
  select(survey_recorded_date,
survey_response_id,
school_name,
school_id,
school_city,
...
#export(org, "clean_dac_22_wide.csv")
#export(org_cols, "colnames.csv")

```

Large data file was subset into the following smaller datasets:  
* Demographics  
* School information  
...
```{r subsets}
core_dat <- import(here("data", "clean_dac_22_wide.csv"))

demographic_data <- core_dat %>% 
  select(school_id,
         leader_tenure,
         confidential_leader_race,
         confidential_leader_race_self_describe,
...
practices_data <- core_dat %>% 
  select(school_id, starts_with("practices"), starts_with("core"), starts_with("time"))

learning_model_data <- core_dat %>% 
  select(school_id,
         learning_model,
         focus_swd,
         focus_emergent_bilinguals,
...
clean_data_full <- import(here("data", "clean_dac_22_wide.csv"))
