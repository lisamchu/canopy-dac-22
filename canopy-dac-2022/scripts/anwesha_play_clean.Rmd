...
```{r libraries}
library(pacman)
p_load(tidyverse, here, rio, janitor, kableExtra, psych)
```
...
## Practice count by focus group -- this is where I'm struggling!
...
```{r}
df %>% 
  group_by(nces_id, focus) %>% 
  summarise(n = length(focus))
```
